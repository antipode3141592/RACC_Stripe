...
function racc_stripe_email_editor_page($atts, $content = null){
	...
	?>
	<div>
		<form method="post" id="racc_email_editor_form" action="/email_editor_update.php">
			<h1>Email Templates</h1>
			<p>Type: <?php _e($db_template_type) ?></p>
			<div>
				<input type="text" id="email_subject_input" class="text_input" value="<?php _e($db_email_subject); ?>"/>
				<label for="email_subject_input">Subject Line</label>
			</div>
			<div>
				<textarea id="email_body_input"><?php _e($db_email_body); ?></textarea>
				<label for="email_body_input">Body Text</label>
			</div>
			<input type="hidden" id="action" value="email_editor"/>
			<input type="hidden" id="template_type_input" value="<?php _e($db_template_type) ?>"/>
			<input type="hidden" id="id_input" value="<?php _e($db_id)?>"/>
